	...
	LinkableValueNode(value.get_type())
{
	...
	else if(value.get_type() == type_real) // for lenght bone
	{
...
ValueNode_IK::operator()(Time t)const
{
	...
	if (joint_mode==Joint::TWOBONE) {
		if (output_bone_index == TargetBone::BONE1){
			if (type == type_real){	
				Real total_L = L1 + L2;
				if (dist > total_L){
					Real dl = dist-total_L;
					return L1 +(dl*L1/total_L); // lenght bone 1
					}
				else return L1;
				}
		...
		if (output_bone_index == TargetBone::BONE2){
			...
			if (type == type_real){	
				...
				if (dist > total_L){
					Real dl = dist-total_L;
					return L2 +(dl*L2/total_L); // lenght bone 2
					}
				else return L2;
				}
	...
	if (output_bone_index == TargetBone::BONE1){
		...
		if(type == type_vector)return Vector(x0 + L1 * std::cos(theta1b), y0 + L1 * std::sin(theta1b));// elbow_1
		}
